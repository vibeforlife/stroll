...
function openMemories(){
  // If the settings modal is open, close it so the memories overlay is visible/clickable.
  closeSettings?.();
...
function clearMemories(){
  // Clear saved moments for this device.
  setMemories([]);
  updateMemUI();
  // If memories overlay is open, refresh it.
  renderMemoriesList();
...
// --- Robust delegated tap handling (fixes mobile modal click-drop issues) ---
let __lastTouchTs = 0;
function __handleDelegated(e){
  const t = (sel)=> e.target && e.target.closest ? e.target.closest(sel) : null;

  if(t("#btnCloseSettings") || t("#btnDoneSettings")){ try{ e.preventDefault(); }catch(_e){} closeSettings(); return; }

  if(t("#btnOpenMemories")){ try{ e.preventDefault(); }catch(_e){} closeSettings(); openMemories(); return; }
  if(t("#btnClearMemories")){ try{ e.preventDefault(); }catch(_e){} clearMemories(); return; }
  if(t("#btnSouvenirPdf")){ try{ e.preventDefault(); }catch(_e){} closeSettings(); generateSouvenirPack(); return; }

  if(t("#btnCloseMemories")){ try{ e.preventDefault(); }catch(_e){} closeMemories(); return; }
  if(t("#btnExportMemories")){ try{ e.preventDefault(); }catch(_e){} exportMemories(); return; }

  if(t("#btnSaveMoment")){ try{ e.preventDefault(); }catch(_e){} saveCurrentMoment(); return; }
}
document.addEventListener("touchend", (e)=>{ __lastTouchTs = Date.now(); __handleDelegated(e); }, {passive:false});
document.addEventListener("click", (e)=>{ if(Date.now() - __lastTouchTs < 650) return; __handleDelegated(e); });
...
// Format a duration (in seconds) into a friendly walk-time string.
// Examples: 540 -> "9 min"; 3900 -> "1 hr 05 min".
function formatDurationFromSeconds(seconds){
  const s = Math.max(0, Math.round(Number(seconds) || 0));
  const totalMin = Math.round(s/60);
  const hr = Math.floor(totalMin/60);
  const min = totalMin % 60;
  if(hr <= 0) return `${totalMin} min`;
  return `${hr} hr ${String(min).padStart(2,"0")} min`;
}
...
function setRoute(idx){
  ...
  const km = approxRouteKm(r.line);
  const secs = (km*1000) / getWalkMps();
  $("distancePill").textContent = `~${km.toFixed(1)} km • ~${formatDurationFromSeconds(secs)}`;
  renderMapForRoute(r);
...
bindTap(bm, openMemories);
const bclr = $("btnClearMemories");
bindTap(bclr, clearMemories);
[...document.querySelectorAll("#btnSouvenirPdf")].forEach(el=>bindTap(el, generateSouvenirPack));

  const bsave = $("btnSaveMoment");
  bindTap(bsave, saveCurrentMoment);

  const bcloseM = $("btnCloseMemories");
  bindTap(bcloseM, closeMemories);

  const bexport = $("btnExportMemories");
  bindTap(bexport, exportMemories);

  // View toggle
  const vStops = $("btnViewStops");
  const vMap = $("btnViewMap");
  if(vStops) vStops.addEventListener("click", ()=> setViewMode("stops"));
  if(vMap) vMap.addEventListener("click", ()=> setViewMode("map"));

  // Restore last view mode
  try{ setViewMode(localStorage.getItem("stroll_view_mode_v1") || "stops"); }catch(e){ setViewMode("stops"); }

  bindTap(bs, openSettings);
  const bc = $("btnCloseSettings");
  bindTap(bc, closeSettings);
  const bd = $("btnDoneSettings");
  bindTap(bd, closeSettings);
  const backdrop = $("settingsBackdrop");
  if(backdrop) backdrop.addEventListener("click", (e)=>{ if(e.target === backdrop) closeSettings(); });
  // Close modals on Escape
  window.addEventListener("keydown", (e)=>{ if(e.key === "Escape"){ closeSettings(); closeMemories(); } });
...
function setViewMode(mode){
  ...
  // Tiny visual indicator: highlight active toggle
  const vStops = $("btnViewStops");
  const vMap = $("btnViewMap");
  if(vStops && vMap){
    const isMap = (mode === "map");
    vStops.classList.toggle("active", !isMap);
    vMap.classList.toggle("active", isMap);
    vStops.setAttribute("aria-pressed", String(!isMap));
    vMap.setAttribute("aria-pressed", String(isMap));
  }
  try{ localStorage.setItem("stroll_view_mode_v1", mode); }catch(e){}
  // Leaflet needs invalidateSize when shown
  if(mode === "map" && map){ setTimeout(()=>{ try{ map.invalidateSize(); }catch(e){} }, 60); }
}
